...
async def get_user_settings(from_user):
    ...
    if (
        ...
        or "as_caption" not in user_dict
        and config_dict.get("LEECH_AS_CAPTION")
    ):
        leech_caption_mode = "Original"
    else:
        leech_caption_mode = "Filename"
    # +++ КОНЕЦ ИЗМЕНЕНИЙ +++
...
<code>Leech Caption    :</code> <b>{leech_caption_mode}</b>
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        if (
            ...
            buttons.data_button(
                "ᴄᴀᴘᴛɪᴏɴ:\nꜰɪʟᴇɴᴀᴍᴇ",
                f"userset {user_id} as_caption false"
            )
        else:
            leech_caption_mode = "Filename"
            buttons.data_button(
                "ᴄᴀᴘᴛɪᴏɴ:\nᴏʀɪɢɪɴᴀʟ",
                f"userset {user_id} as_caption true"
            )
        if (
            IS_PREMIUM_USER
            and user_dict.get(
